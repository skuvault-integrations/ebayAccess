	...
	public class EbayItemsParser
	{
		...
		public List<Item> ParseGetSallerListResponse( Stream stream )
		{
			try
			{
				...
				IEnumerable<XElement> xmlItems = root.Descendants( ns + "Item" );

				List<Item> orders = xmlItems.Select( x =>
				{
					object temp = null;
					var res = new Item();
					...
					XElement listingDetails = x.Element( ns + "ListingDetails" );
					...
					XElement primaryCategory = x.Element( ns + "PrimaryCategory" );
